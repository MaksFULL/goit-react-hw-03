...
const App = () => {
  const initContacts = () => {
    ...
    else { return JSON.parse(localStorage.getItem("contacts")) };
  }

  const [contactList, setContactList] = useState(initContacts);
  useEffect(() => localStorage.setItem("contacts", JSON.stringify(contactList)), [contactList]);

  const [search, setSearch] = useState("");
  const filteredContacts = contactList.filter(contact => contact.name.toLowerCase().includes(search));
